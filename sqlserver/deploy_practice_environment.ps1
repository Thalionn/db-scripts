...
function Test-SqlConnection
{
    ...
    try
    {
        # Try dbatools module first (cross-platform)
        ...
        if ($module)
        {
            ...
            $conn = Connect-DbaInstance -SqlInstance $ServerInstance -SqlCredential $cred -TrustServerCertificate -ErrorAction Stop
            Write-Host "Successfully connected to: $ServerInstance ($($conn.ServerVersion))" -ForegroundColor Green
            $conn.Disconnect()
            return $true
        }
...
function Execute-SqlCommand
{
    ...
    # Use dbatools if available (cross-platform)
    $module = Get-Module -Name dbatools -ListAvailable
    if ($module)
    {
        $cred = $null
        ...
        $result = Invoke-DbaQuery -SqlInstance $ServerInstance -Database $DatabaseName -SqlCredential $cred -Query $SqlScript -TrustServerCertificate
        if ($result -is [System.Data.DataTable]) { return $result }
...
if (-not $SkipDownload)
{
    ...
    # 4. Download WideWorldImporters demo database (BACPAC format)
    Write-Host "`n[4/4] Downloading WideWorldImporters demo..." -ForegroundColor Cyan

    try
    {
        $wwiDir = Join-Path $OutputPath "WideWorldImporters"
        if (-not (Test-Path $wwiDir))
        {
            New-Item -ItemType Directory -Path $wwiDir -Force | Out-Null

            $wwiBakUrl = "https://github.com/Microsoft/sql-server-samples/releases/download/wide-world-importers-v1.0/WideWorldImporters-Full.bak"

            # Also try Standard if Full is too large
            $bakFile = Join-Path $wwiDir "WideWorldImporters-Full.bak"

            Write-Host "Downloading WideWorldImporters backup (2GB+)..." -ForegroundColor Green
            Write-Host "URL: $wwiBakUrl" -ForegroundColor Gray
            try {
                Invoke-WebRequest -Uri $wwiBakUrl -OutFile $bakFile -TimeoutSec 1800
            } catch {
                Write-Host "Full backup download failed, trying Standard..." -ForegroundColor Yellow
                $wwiBakUrl = "https://github.com/Microsoft/sql-server-samples/releases/download/wide-world-importers-v1.0/WideWorldImporters-Standard.bak"
                $bakFile = Join-Path $wwiDir "WideWorldImporters-Standard.bak"
                Invoke-WebRequest -Uri $wwiBakUrl -OutFile $bakFile -TimeoutSec 1800
            }

            if (Test-Path $bakFile) {
                Write-Host "Downloaded: $bakFile" -ForegroundColor Green
                Write-Host "Manual restore command:" -ForegroundColor Gray
                Write-Host "  RESTORE DATABASE WideWorldImporters FROM DISK = '$bakFile' WITH MOVE 'WWI_Primary' TO '...WWI_Primary.mdf', MOVE 'WWI_Log' TO '...WWI_Log.ldf'" -ForegroundColor Gray
            }
        } else
        {
...
if (Test-Path $sqlScriptsPath)
{
    $sqlFiles = Get-ChildItem -Path $sqlScriptsPath -Filter "*.sql" -Recurse |
        Where-Object { $_.Name -ne "practice_environment_setup.sql" -and $_.Name -ne "06_login_trigger.sql" } |
        Sort-Object Name
